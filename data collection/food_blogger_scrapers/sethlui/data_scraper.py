...
def get_search_links(page, keyword, name):
    browser.get("https://sethlui.com/page/%d/?s=%s" % (page,keyword))
    WebDriverWait(browser, 30).until(is_ready)
    main_content = browser.find_element(By.ID, "tdi_62")
    results = main_content.find_elements(By.CLASS_NAME, "td-module-title")
    matching_results = []

    for result in results:
        links = result.find_elements(By.TAG_NAME, "a")
        link = links[0].get_attribute("href")
        link_text = links[0].get_attribute("title")
...
def crawl_data(page, keyword, name, result_df):
    ...
    for i in range(len(matching_results)):
        ...
        main_content = browser.find_element(By.CLASS_NAME, "td-main-content-wrap")
